...
if getattr(sys, 'frozen', False):
    # 実行ファイル化された場合、sys.executable のディレクトリを使用
...
SERVER_DLL = os.path.join(DATA_DIR, "server.dll")
WEB_DLL = os.path.join(DATA_DIR, "web.dll")
VERSION_TXT = os.path.join(DATA_DIR, "version.txt")
INSTALL_TXT = os.path.join(DATA_DIR, "install.txt")
...
def download_and_prepare_html():
    # WEB_DLLをdataフォルダー内のweb.dllとして扱う
...
def update():
    try:
        ...
        if os.path.exists(VERSION_TXT):
            with open(VERSION_TXT, 'r', encoding='utf-8') as vf:
        ...
        for update in reversed(updates):
            ...
            # version.txtへの追記
            with open(VERSION_TXT, 'a', encoding='utf-8') as vf:
...
if __name__ == "__main__":
    # SERVER_DLL が存在しなければ、デフォルトリンクを書き込む
